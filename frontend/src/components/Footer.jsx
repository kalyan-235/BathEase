...
// Public endpoint — no auth needed, any visitor can fetch admin contact
function useAdminContact() {
  const [contact, setContact] = useState({ whatsapp: '', email: 'support@bathease.in' });
...
export function Footer() {
  const { whatsapp, email } = useAdminContact();
  const displayWa = whatsapp || '+91 99999 99999';
  const waHref    = `https://wa.me/${displayWa.replace(/[^\d]/g, '')}`;
  ...
  return (
    <footer className="mt-16 border-t border-border/60 bg-muted/30 pb-20 md:pb-0">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-8">
        ...
        {/* Support — shows admin's live WhatsApp */}
        <div>
